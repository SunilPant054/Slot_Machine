...
def deposit():
    while True:
        ...
        if amount.isdigit(): #checks whether the amount is a number
            amount = int(amount)
            if amount > 0:
                break
            else:
...
def main():
    ...
    #Check if the bet is inside the deposited amount
    while True: 
        bet = get_bet()
        total_bet = bet * lines
        
        if total_bet > balance:
            print(f"You donot have enought balance to bet, youre current balance is {balance}")
        else:
            break
